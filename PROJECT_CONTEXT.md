...
**Current Phase:** Phase A + E complete: Google Sheets CSV connected + catalog XSS render guard
**Progress:** ~70% (แคตตาล็อกใช้ CSV จริงแล้ว เหลือ UX ค้นหา/ตะกร้าแยกกลุ่ม/ส่งราคาไป production)
**Next Session:** ทำ Phase B (ช่องค้นหา + ปรึกษาแอดมินเมื่อไม่พบสินค้า) ตาม `implementation_plan.md`
...
|-------|-------|----------|-------------|
| Phase B | ช่องค้นหา + fallback "ปรึกษาแอดมิน" เมื่อไม่พบสินค้า | - | รอบถัดไป |

### ยังไม่ได้ทำ (Pending — เรียงตามที่คุยไว้)
| Task | Priority | หมายเหตุ |
|------|----------|----------|
| ช่องค้นหา + fallback "ปรึกษาแอดมิน" | High | ทำเป็น Phase B ตาม `implementation_plan.md` |
...
1. ทำ Phase B: ช่องค้นหา + fallback "ปรึกษาแอดมิน" เมื่อไม่พบสินค้าในแคตตาล็อก
2. ทำ Phase C: ตะกร้า 2 กลุ่ม (สั่งซื้อ/จองคิว แยกยอดรวม+ปุ่มเช็คเอาต์)
3. วางแผน Phase D1 แยกรอบ: ส่งราคาอ้างอิงไป production form ผ่าน URL param (ต้องคุยกับ CEO ก่อน deploy production)
4. ปิดช่องโหว่ upload validation ฝั่ง server ใน production repo แยกรอบ ไม่ใช่ scope ของ landing page รอบนี้
